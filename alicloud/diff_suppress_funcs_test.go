...
func TestUnitCommonPolardbServrelessTypeDiffSuppressFunc(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			serverlessType: "AgileServerless",
			expected:       false,
			description:    "PostgreSQL 15 with AgileServerless should not suppress diff",
		},
		{
			name:                   "PostgreSQL_15_SteadyServerless_ON",
			dbType:                 "PostgreSQL",
			dbVersion:              "15",
			serverlessType:         "SteadyServerless",
			serverlessSteadySwitch: "ON",
			expected:               true,
			description:            "PostgreSQL 15 SteadyServerless is outside the supported compatibility matrix",
		},
		{
			name:           "PostgreSQL_16_AgileServerless",
			dbType:         "PostgreSQL",
			dbVersion:      "16",
			serverlessType: "AgileServerless",
			expected:       true,
			description:    "PostgreSQL 16 AgileServerless is outside the supported compatibility matrix",
		},
		{
